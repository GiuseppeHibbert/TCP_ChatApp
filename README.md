...
6. Now that you have the server hosted, and you know the IP and PORT of the server via steps 3 and 4, we can login.
To login, switch to the client terminal, and type in:
LOGIN <IP_OF_SERVER> <PORT_OF_SERVER>
7. If you type in an invalid IP address or PORT when attempting to log in, you will receive this message:
<img src=images/6_badLoginPort.png></img>
8. Once you are logged in, you can see the currently connected users via the LIST command:
<img src=images/5_getList.png></img>
9. To refresh the list of logged in users, use the REFRESH command:
<img src=images/7_refreshList.png></img>
10. To send a message to another user, first get their IP from the LIST command, then use the SEND command as shown below:
<img src=images/9_messageRecorded.png></img>
11. To broadcast a message to every user connected, use the BROADCAST command as shown below:
<img src=images/10_broadcast.png></img>
12. To logout as a client from the server, use the LOGOUT command as shown below:
<img src=images/11_logout.png></img>
13. You must be logged in to logout, as shown below:
<img src=images/12_logoutexception.png></img>
14. Finally to close down the client, use the EXIT command:
